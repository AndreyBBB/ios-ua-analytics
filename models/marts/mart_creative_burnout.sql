...
{{
  config(
    ...
    order_by = ['creative_id', 'stat_date']
  )
}}
...
select
    ...
    case
        ...
        when be.burnout_date is null
            ...
            then 'peak'
        when d.ctr_7d_avg >= (p.absolute_peak_ctr * {{ var('burnout_ctr_threshold') }})
            then 'declining'
        else 'burnt'
...
left join {{ ref('int_burnout_events') }} be
